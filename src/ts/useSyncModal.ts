import {
  salvarAlunosOffline,
  salvarEscolasOffline,
  salvarProvaOffline,
  salvarTurmasOffline,
} from "@/src/database/services/salvarAvaliacao";
import {
  listaAlunos,
  listarEscolas,
  listaTurmas,
...
export function useSyncModal() {
  ...
  async function handleBaixarProvas() {
    if (selecionados.length === 0) {
      Alert.alert(
        "Atenção",
        "Por favor, selecione pelo menos uma prova para baixar.",
      );
      return;
    }
    ...
    try {
      ...
      const idAvaliacaoSaed = provasParaSalvar[0]?.id_avaliacao_saed;
      console.log("numero id", idAvaliacaoSaed);
      if (!idAvaliacaoSaed) {
        Alert.alert(
          "Atenção",
          "Não foi possível identificar o cliente da prova.",
        );
        return;
      }
      ...
      const [alunosRes, escolasRes, turmaRes] = await Promise.all([
        listaAlunos({ id_avaliacao_saed: idAvaliacaoSaed }),
        listarEscolas({ id_avaliacao_saed: idAvaliacaoSaed }),
        listaTurmas({ id_avaliacao_saed: idAvaliacaoSaed }),
      ]);

      // 🔍 1. INSspecione o retorno da API no seu console
      console.log(
        "Qtd de Alunos recebidos da API:",
        Array.isArray(alunosRes) ? alunosRes.length : 0,
      );
      console.log("Exemplo do primeiro aluno:", alunosRes?.[0]);

      console.log(
        "Qtd de escolas recebidos da API:",
        Array.isArray(escolasRes) ? escolasRes.length : 0,
      );
      console.log("Exemplo do primeiro escola:", escolasRes?.[0]);

      console.log(
        "Qtd de turmas recebidos da API:",
        Array.isArray(turmaRes) ? turmaRes.length : 0,
      );
      console.log("Exemplo do primeiro turma:", turmaRes?.[0]);
      ...
      Alert.alert(
        "Erro no download",
        "Houve uma falha ao tentar salvar os dados no dispositivo local.",
      );
    } finally {
      setBaixando(false);
    }
